...
// Fetch NCMC data from database
async function fetchNCMCData() {
  try {
    ...
    if (missionError || !mission) {
      console.error('Error fetching NCMC mission:', missionError);
      initializeWithFallbackData();
      return;
    }

    NCMC_MISSION_ID = mission.id;

    const { data: districts, error: districtError } = await sb
      .from('districts')
      .select('id')
      .eq('mission_id', NCMC_MISSION_ID);
    ...
    console.error('Error fetching NCMC data:', error);
    initializeWithFallbackData();
  }
}
